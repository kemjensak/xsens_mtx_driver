...
int main(int argc, char **argv)
{
	...
	float R_s_h_stand_T[3][3];
	for (int i = 0; i < 3; i++)
	{
		for (int j = 0; j < 3; j++)
		{
			R_s_p_stand_T[i][j] = R_s_p_stand[j][i];
			R_s_h_stand_T[i][j] = R_s_h_stand[j][i];
		}
	}
	...
	struct rotation_matrix struct_R_b_f;
	struct rotation_matrix struct_R_p;
	struct rotation_matrix struct_R_h;
	float R_p[3][3];
	float R_h[3][3];
	float R_h_T[3][3];
	float R_c_p[3][3];
	...
	float imu_h_acc[3][1];
	float imu_h_acc_ing[3][1];
	float non_gravity_acc[3][1];
	float gravity[3][1] = { {0},
							{0},
							{9.81} };

	struct rotation_matrix R1;
	int angle = 30;

	FILE* fp_stoop;
	FILE* fp_stand;
	FILE* fp_hand_stoop;
	fp_stoop = fopen("stoop.txt", "w");
	fp_stand = fopen("stand.txt", "w");
	fp_hand_stoop = fopen("hand_stoop.txt", "w");

	struct_R_b_f = make_R_b_f();

	R_b_f[0][0] = struct_R_b_f.r1;
	R_b_f[0][1] = struct_R_b_f.r2;
	R_b_f[0][2] = struct_R_b_f.r3;
	R_b_f[1][0] = struct_R_b_f.r4;
	R_b_f[1][1] = struct_R_b_f.r5;
	R_b_f[1][2] = struct_R_b_f.r6;
	R_b_f[2][0] = struct_R_b_f.r7;
	R_b_f[2][1] = struct_R_b_f.r8;
	R_b_f[2][2] = struct_R_b_f.r9;

	for (int i = 0; i < 3; i++) {
		for (int j = 0; j < 3; j++) {
			for (int k = 0; k < 3; k++) {
				R_c_p[i][j] += R_s_p_stand_T[i][k] * R_b_f[k][j];
				R_c_h[i][j] += R_s_h_stand_T[i][k] * R_b_f[k][j];
			}
		}
	}

	ros::init(argc, argv, "mtx_cal");
	ros::NodeHandle nh;
